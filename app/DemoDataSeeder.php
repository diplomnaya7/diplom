...
<p>Не открывайте ссылки от неизвестных источников. Это главная входная точка для вирусов.</p>',
            ...
            [
                ...
                'content' => '<p>Понимание основ SEO критично для привлечения органического трафика. Разберём метатеги, структуру заголовков и оптимизацию контента.</p>',
                'excerpt' => 'Краткое руководство по базовой SEO-оптимизации сайта.',
                'category_id' => $categoryIds[1] ?? 2,
                'author_id' => $authorId,
                'status' => 'published',
            ],
            [
                ...
                'content' => '<p>Пошаговые рекомендации по сокращению времени загрузки: сжатие, кэширование, оптимизация изображений и lazy-load.</p>',
                'excerpt' => 'Методы ускорения веб-страниц и улучшения UX.',
                'category_id' => $categoryIds[0] ?? 1,
                'author_id' => $authorId,
                'status' => 'published',
            ],
            [
                ...
                'content' => '<p>Доступность помогает сделать контент доступным для всех пользователей — обсуждаем ARIA, семантическую разметку и клавиатурную навигацию.</p>',
                'excerpt' => 'Ключевые практики веб-доступности для разработчиков и авторов.',
                'category_id' => $categoryIds[0] ?? 1,
                'author_id' => $authorId,
                'status' => 'published',
            ],
            [
                ...
                'content' => '<p>Обзор подходов к тестированию веб-приложений: юнит-тесты, интеграционные и end-to-end тесты с практическими примерами.</p>',
                'excerpt' => 'Как построить стратегию тестирования для надёжного сайта.',
                'category_id' => $categoryIds[0] ?? 1,
                'author_id' => $authorId,
                'status' => 'published',
            ],
            [
                ...
                'content' => '<p>Простые и эффективные стратегии резервного копирования: куда сохранять бэкапы, частота, автоматизация и тест восстановления.</p>',
                'excerpt' => 'Практическое руководство по защите данных сайта.',
                'category_id' => $categoryIds[4] ?? 5,
                'author_id' => $authorId,
                'status' => 'published',
            ],
            [
                'title' => 'Как настроить CI/CD для PHP-проекта',
                'slug' => 'nastrojka-ci-cd-dlya-php',
                'content' => '<p>Интеграция CI/CD повышает качество релизов. Разберём примеры конфигураций для GitHub Actions и простую пайплайн-логику для тестов и деплоя.</p>',
                'excerpt' => 'Быстрый старт с CI/CD для PHP-проектов.',
                'category_id' => $categoryIds[0] ?? 1,
                'author_id' => $authorId,
                'status' => 'published',
            ],
            [
                'title' => 'Кэширование: CDN, HTTP и серверные техники',
                'slug' => 'keshirovanie-cdn-http-server',
                'content' => '<p>Различные уровни кэширования и примеры их применения: CDN, заголовки HTTP, серверный кеш и инвалидация.</p>',
                'excerpt' => 'Как правильно кэшировать ресурсы для скорости и масштаба.',
                'category_id' => $categoryIds[0] ?? 1,
                'author_id' => $authorId,
                'status' => 'published',
            ],
            [
                'title' => 'Защита от SQL-инъекций и безопасные запросы',
                'slug' => 'zashchita-ot-sql-injekcij',
                'content' => '<p>Почему SQL-инъекции опасны и как им противостоять: подготовленные выражения, валидация и минимизация прав доступа.</p>',
                'excerpt' => 'Базовые и продвинутые техники защиты БД.',
                'category_id' => $categoryIds[4] ?? 5,
                'author_id' => $authorId,
                'status' => 'published',
            ],
            [
                ...
                'slug' => 'optimizaciya-izobrazhenij-dlya-veba',
                'content' => '<p>Форматы, сжатие и выбор размеров изображений — как снизить трафик и ускорить загрузку без потери качества.</p>',
                'excerpt' => 'Практические советы по работе с изображениями на сайте.',
                'category_id' => $categoryIds[0] ?? 1,
                'author_id' => $authorId,
                'status' => 'published',
            ],
            [
                'title' => 'Аналитика сайта: какие метрики отслеживать',
                'slug' => 'analitika-kakie-metriki-otslezhivat',
                'content' => '<p>Список важных метрик: посещаемость, вовлечённость, источники трафика и конверсии. Как правильно интерпретировать данные.</p>',
                'excerpt' => 'Ключевые метрики для оценки эффективности блога.',
                'category_id' => $categoryIds[1] ?? 2,
                'author_id' => $authorId,
                'status' => 'published',
            ],
            [
                'title' => 'Работа в команде: git-воркфлоу для контент-проектов',
                'slug' => 'git-workflow-dlya-kontent-proektov',
                'content' => '<p>Организация совместной работы над контентом и кодом: ветвление, ревью и автоматические проверки перед мерджем.</p>',
                'excerpt' => 'Практики git-воркфлоу для команд редакции и разработчиков.',
                'category_id' => $categoryIds[1] ?? 2,
                'author_id' => $authorId,
                'status' => 'published',
            ],
